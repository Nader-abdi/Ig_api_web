...
class Instagram
{
    ...
    private $userSession;
    private $rhxGis = null;
    private $userAgent = 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/84.0.4147.105 Safari/537.36';
    private $headers;
...
//        return md5(implode(':', [$this->getRhxGis(), $variables]));
     ...
     * @throws InstagramNotFoundException
     */
    public function getFollowers($accountId, $count = 20, $pageSize = 20, $delayed = true, $reels = true, $maxId = null)
    {
        $result = $this->getPaginateFollowers($accountId, $count, $pageSize, $delayed, $reels , $maxId);
     ...
     * @throws InstagramNotFoundException
     */
    public function getPaginateFollowers($accountId, $count = 20, $pageSize = 20, $delayed = true, $reels = true, $nextPage = null)
    {
        if ($delayed) {
        ...
        while (true) {
            ...
            if ($reels) {
                $variables = json_encode([
                    'id' => (string)$accountId,
                    'include_reel' => (string)true,
                    'fetch_mutual' => (string)false,
                    'first' => (string)$pageSize,
                    'after' => (string)$endCursor
                ]);
                $response = Request::get(Endpoints::getFollowersJsonLink($accountId, $pageSize, $endCursor,$variables),
                    $this->generateHeaders($this->userSession));
            } else {
        ...
        $toReturn['accounts']['maxId'] = $endCursor;
        return $toReturn;
    }
    ...
    public function login($force = false, $twoStepVerificator = null)
    {
        ...
        if(!empty($this->customCookies)){
            ...
            if(!empty($session) and is_array($session) and array_key_exists("sessionid",$session)){

                foreach ($session as $key => $value){
                    if($session[$key] == "sessionid" and !empty($session[$key])){
                        $session[$key] = $this->customCookies["sessionid"];
                    }
                }
            }else{
                $session["sessionid"] = $this->customCookies["sessionid"];
            }
        }
        if ($force || !$this->isLoggedIn($session)) {
            ...
            $cookies['mid'] = $mid;
            static::$instanceCache->set($this->getCacheKey(), $cookies,3000000000);
    ...
    public function isLoggedIn($session)
    {
        ...
        $this->setHeader('cookie', $this->cookie_string($session));
        $this->setHeader('referer',Endpoints::BASE_URL . '/');
        if(isset($session['csrftoken'])){
            $csrfToken = $session['csrftoken'];
        }else{
            $this->setHeader('cookie', $this->cookie_string($session));

            $response = Request::get(Endpoints::BASE_URL, $this->headers);
            if ($response->code !== static::HTTP_OK) {
                return false;
            }
            $cookies = $this->parseCookies($response->headers);
        ...
        $session = $this->cookie_session_add($cookies,$session);


        static::$instanceCache->set($this->getCacheKey(),$session,300000);
    ...
    {
        static::$instanceCache->set($this->getCacheKey(), $this->userSession,3000000);
    ...
    public function cookie_string($cookie){
        if(is_array($cookie)){
            $cookieStrings = "";
    ...
    public function cookie_session_add($cookie , $session)
    {
        foreach ($cookie as $key => $value){
            $session[$key] = $value;
        }
